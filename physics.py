...
class VirtualArea:
    def __init__(self, up_x, up_y, down_x, down_y):
        ...
        self.ArrBorder = [[max(up_x, down_x), max(up_y, down_y)],
                           [min(up_x, down_x), min(up_y, down_y)]]

    def get_border(self):
        return self.ArrBorder

    def is_in_border(self, x, y, radius):
        return self.ArrBorder[1][0] + radius < x < self.ArrBorder[0][0] - radius \
                and self.ArrBorder[1][1] + radius < y < self.ArrBorder[0][1] - radius
    ...
    def resolve_ball_collisions(self):
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                # Если шары пересекаются (произошло столкновение)
                if distance < min_dist:
                    # Защита от деления на ноль, если центры совпали идеально
                    if distance == 0:
                        dx = 1.0
                        dy = 0.0
                        distance = 1.0
